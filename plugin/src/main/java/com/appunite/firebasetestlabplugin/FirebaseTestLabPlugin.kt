    ...
    private fun createDownloadSdkTask(project: Project, cloudSdkPath: String?): Sdk =
            if (cloudSdkPath != null) {
                ...
                project.tasks.register<Task>(ensureGCloudSdk) {
                    group = com.appunite.firebasetestlabplugin.utils.Constants.FIREBASE_TEST_LAB
                ...
                project.tasks.register<HiddenExec>(ensureGCloudSdk) {
                    ...
                    commandLine = kotlin.collections.listOf("bash", "-c", "rm -r \"${cloudSdkDir.absolutePath}\";export CLOUDSDK_CORE_DISABLE_PROMPTS=1 && export CLOUDSDK_INSTALL_DIR=\"${installDir.absolutePath}\" && curl https://sdk.cloud.google.com | bash")
                    doLast {
...
private fun resolveApk(variant: ApkVariant, baseVariantOutput: BaseVariantOutput): File =
        try {
            ...
            applicationProvider.let {
                ...
                File(it.outputDirectory.asFile.get(), filename)
            }
        } catch (e: Exception) {
            when (e) {
